...
namespace VHierarchy
{
    public static class VHierarchy
    {
        ...
        static void RowGUI(int instanceId, Rect rowRect)
        {
            ...
            if (VHierarchyMenu.componentCustomButtonGaspar)
            {
                Rect buttonRect = new Rect(rowRect.xMax - 20f, rowRect.y, 18f, rowRect.height - 2f);
                if (GUI.Button(buttonRect, "B"))
                {
                    // Get the GameObject associated with the button click
                    GameObject obj = EditorUtility.InstanceIDToObject(instanceId) as GameObject;

                    if (obj != null)
                    {
                        Debug.Log($"Button clicked for GameObject: {obj.name}");
